...
describe('skill registry', () => {
    ...
    it('lists all registered agent names', () => {
        expect(listAgents()).toEqual(AGENTS.map((agent) => agent.agent))
    })
})
...
describe('installer paths', () => {
    it.each(AGENTS)(
        ...
        ({ agent, description, dirName }) => {
            ...
            expect(installer.getInstallPath(false)).toBe(
                join(homedir(), dirName, 'skills', 'granola-cli', 'SKILL.md'),
            )
            expect(installer.getInstallPath(true)).toBe(
                join(process.cwd(), dirName, 'skills', 'granola-cli', 'SKILL.md'),
            )
        },
    )
})
...
describe('install detection', () => {
    ...
    it('skips agent directory check for universal (.agents)', async () => {
        const testDir = join(tmpdir(), `granola-cli-test-${Date.now()}`)
