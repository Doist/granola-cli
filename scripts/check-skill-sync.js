...
/**
 * Verifies that skills/granola-cli/SKILL.md is in sync with the generated
...
try {
    ...
    const actual = await readFile(join(root, 'skills/granola-cli/SKILL.md'), 'utf-8')

    if (actual !== expected) {
        console.error(
            'ERROR: skills/granola-cli/SKILL.md is out of sync with src/lib/skills/content.ts',
        )
        console.error('')
    ...
    console.log('skills/granola-cli/SKILL.md is in sync with content.ts')
} catch (error) {
